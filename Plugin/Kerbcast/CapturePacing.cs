...
// and that choice is load-bearing. A background camera joins a tick's capture set only
// once its interval has elapsed, so it consumes stagger permits in proportion
// to its rate — a 1fps background camera among 30fps primaries takes about 1/30th of
// the slots. Since background capture varies rate and never resolution, a background capture capture
...
namespace Kerbcast
{
    ...
    public struct CapturePacing
    {
        ...
        /// until the first, so a newly-capturing in the background camera is due immediately.</summary>
        ...
        /// always eligible; a background camera only once its interval has elapsed.</summary>
